...
#rm -f start.sh && curl -s -f -L --retry 3 --connect-timeout 3 --max-time 10 --retry-delay 1 --raw -o start.sh http://192.168.0.12:5500/start.sh && chmod +x start.sh && ./start.sh
...
#https://github.com/ajay9634/Ajay-prefix/releases/download/Box64_wcp/box64-0.3.1.8g2.wcp
...
cmake --install-prefix $PREFIX/glibc .. -DARM_DYNAREC=ON -DCMAKE_BUILD_TYPE=RelWithDebInfo -DSD8G2=ON -DBAD_SIGNAL=ON -DGLES=ON -DVULKAN=ON -DARM64_DYNAREC_PASS=ON -DARM64_DYNAREC_BIGBLOCK=ON
